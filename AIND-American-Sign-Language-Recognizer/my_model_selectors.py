    ...
    def select(self):
        ...
        for n_comp in range(self.min_n_components,self.max_n_components+1):
            ...
            try:
                ...
                n_state_param = hmm_model.transmat_.shape[0]*(hmm_model.transmat_.shape[1]-1)
                n_obsv_param = n_comp*self.X.shape[1]*2
                n_params =  n_state_param + n_obsv_param + (n_comp-1)
                bic = (-2 * logL)+ (n_params*np.log(self.X.shape[0]))
                if bic < best_bic:
                    best_bic = bic
                    best_n = n_comp
            except:
        ...
        for n_comp in range(self.min_n_components,self.max_n_components+1):
            ...
            try:
                ...
                for word in self.words:
                    ...
                    tot_other_logL += hmm_model.score(other_X, other_length)
                    tot_other_words += 1
                tot_other_logL /= tot_other_words
                dic = logL - tot_other_logL
                if dic >= best_dic:
                    best_dic = dic
                    best_n = n_comp                
            except:
        ...
        for n_comp in range(self.min_n_components,self.max_n_components+1):            
            ...
            for cv_train_idx, cv_test_idx in split_method.split(self.sequences):
                ...
                try:
                    ...
                    tot_cv_score += test_logL
                except:
                    None
            tot_cv_score /= n_splits                
            if tot_cv_score >= best_score:
                best_score = tot_cv_score
                best_n = n_comp        
        return self.base_model(best_n)
